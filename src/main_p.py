...
def master_tweet_processor(comm, input_file):
	...
	total_count_hashtags = {"A1": Counter(), "A2": Counter(), "A3": Counter(), "A4": Counter(),
							"B1": Counter(), "B2": Counter(), "B3": Counter(), "B4": Counter(),
							"C1": Counter(), "C2": Counter(), "C3": Counter(), "C4": Counter(),
							"D1": Counter(), "D2": Counter(), "D3": Counter(), "D4": Counter()}

	total_count_posts, total_count_hashtags1 = process_tweets(rank, input_file, size)
	for key, grid in total_count_hashtags1.items():
		total_count_hashtags[key] = total_count_hashtags1[key] + grid
	...
	for worker_count in counts:
		...
		for key, grid in worker_count[1].items():
			total_count_hashtags[key] = total_count_hashtags[key] + grid
